...
# Filtrando Balança Comercial - Expectativa ---------------------
balanca_comercial_expectativa <- df %>%
...
if (interactive()) View(balanca_comercial_expectativa)

# Igualando a data da expectativa para DataReferencia
balanca_comercial_expectativa$Data <- balanca_comercial_expectativa$Data + years(1)

if (interactive()) View(balanca_comercial_expectativa)

# Obtendo os Dados reais de Balança Comercial ---------------------
balanca_comercial_real <- get_series(22707,
                         start_date = (data_ini %m-% months(12)),
                         end_date   = data_fim) %>%
  `colnames<-`(c("Data", "Balanca_Comercial")) %>%
  arrange(Data) %>%
  mutate(
    Balanca_Comercial = Balanca_Comercial / 1000,                      # milhões -> bilhões
    Balanca_Comercial_12m = zoo::rollapplyr(Balanca_Comercial, 12, sum, fill = NA)  # soma 12m em bilhões
  )
  
if (interactive()) View(balanca_comercial_real)

#--------Grafico----------------------------

# Salvar dados utilizados no gráfico em CSV
dados_expectativas <- balanca_comercial_expectativa %>%
  dplyr::select(Data, Minimo, Maximo, Mediana) %>%
  arrange(Data)

dados_resultado <- balanca_comercial_real %>%
  dplyr::select(Data, Balanca_Comercial_12m) %>%
  arrange(Data)

utils::write.table(dados_expectativas,
                   file = file.path("data", "balanca_comercial_expec.csv"),
                   sep = ";", dec = ",", row.names = FALSE, col.names = TRUE, qmethod = "double")
utils::write.table(dados_resultado,
                   file = file.path("data", "balanca_comercial_real.csv"),
                   sep = ";", dec = ",", row.names = FALSE, col.names = TRUE, qmethod = "double")

# Gráfico
p <- 
  ggplot(balanca_comercial_expectativa, aes(x = Data)) +
  ...
  # Linha da Balança Comercial 12m (US$ bi)
  geom_line(data = balanca_comercial_real,
            aes(x = Data, y = Balanca_Comercial_12m, colour = "Balança comercial 12m"), size = 1.2) +
  
  # Linha vertical da ata
  geom_vline(xintercept = as.numeric(data_ata), linetype = "dashed", colour = "red", size = 1) +
  annotate("text", x = data_ata, y = max(c(balanca_comercial_expectativa$Maximo, balanca_comercial_real$Balanca_Comercial_12m), na.rm = TRUE) * 0.95,
  ...
  annotate("text", x = data_ata_menos12, y = max(c(balanca_comercial_expectativa$Maximo, balanca_comercial_real$Balanca_Comercial_12m), na.rm = TRUE) * 0.95,
  ...
  annotate("text", x = data_ata_mais12, y = max(c(balanca_comercial_expectativa$Maximo, balanca_comercial_real$Balanca_Comercial_12m), na.rm = TRUE) * 0.95,
  ...
  annotate("text", x = data_ata_mais24, y = max(c(balanca_comercial_expectativa$Maximo, balanca_comercial_real$Balanca_Comercial_12m), na.rm = TRUE) * 0.95,
  ...
  scale_colour_manual(name = NULL,
                      breaks = c("Balança comercial 12m", "Mediana (Expectativa)"),
                      values = c("Balança comercial 12m" = "firebrick",
...
ggsave(filename = file.path("data", "9_balanca_comercial.png"), plot = p,
